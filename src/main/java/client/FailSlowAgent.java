...
public class FailSlowAgent{
    ...
    public static void report(Integer type,int id){
        try{
            ...
            if(res.isInject){
                System.out.println("FailSlowAgent.java | isInject : "+res.isInject+" delayTime : "+res.delayTime+" thread id:"+Thread.currentThread().getId());
    ...
    public static void traceNonBlockingIOStartTime(int nodeId){
        try{
            SchedulerService access=(SchedulerService)Naming.lookup("rmi://172.30.0.5:2359/failslow");
            ScheduleInfo info=FailSlowAgent.getInfo();
            info.nodeId=nodeId;
            long startTime=System.currentTimeMillis();
            access.reportNonBlockingStartTime(info,startTime);
        }catch(Exception e){
            e.printStackTrace();
        }
    }

    public static void traceNonBlockingIOEndTime(int nodeId){
        try{
            SchedulerService access=(SchedulerService)Naming.lookup("rmi://172.30.0.5:2359/failslow");
            ScheduleInfo info=FailSlowAgent.getInfo();
            info.nodeId=nodeId;
            long endTime=System.currentTimeMillis();
            access.reportNonBlockingEndTime(info,endTime);
        }catch(Exception e){
            e.printStackTrace();
        }
    }
