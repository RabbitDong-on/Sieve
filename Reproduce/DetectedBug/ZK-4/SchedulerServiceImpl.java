...
public class SchedulerServiceImpl extends UnicastRemoteObject implements SchedulerService{
    ...
    public synchronized ScheduleRes isInjectDelay(ScheduleInfo info) throws RemoteException{
        ...
        if(EngineDCT.isDCT){
            ...
            // System.out.println("SchedulerServiceImpl | check:true");
            if(!info.funcName.equals("serialize")||info.nodeId!=2){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
            if(!info.lastSTE[4].getMethodName().equals("serializeSnapshot")){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
            if(!info.lastSTE[5].getMethodName().equals("serializeSnapshot")){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
            if(!info.lastSTE[6].getMethodName().equals("run")||info.lastSTE[6].getLineNumber()!=583){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
