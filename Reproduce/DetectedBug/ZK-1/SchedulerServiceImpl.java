...
public class SchedulerServiceImpl extends UnicastRemoteObject implements SchedulerService{
    ...
    public synchronized ScheduleRes isInjectDelay(ScheduleInfo info) throws RemoteException{
        ...
        if(EngineDCT.isDCT){
            ...
            if(!info.funcName.equals("sealStream")){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
            if(!info.lastSTE[4].getMethodName().equals("serialize")){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
            if(!info.lastSTE[5].getMethodName().equals("save")){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
            if(!info.lastSTE[6].getMethodName().equals("takeSnapshot")){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
            if(!info.lastSTE[7].getMethodName().equals("takeSnapshot")){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
            if(!info.lastSTE[8].getMethodName().equals("takeSnapshot")){
                res.isInject=false;
                res.delayTime=0;
                return res;
            }
