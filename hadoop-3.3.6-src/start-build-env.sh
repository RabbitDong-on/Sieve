...
docker build -t hadoop-build-XX -f $DOCKER_FILE $DOCKER_DIR
...
docker build -t "hadoop-build-XX-${USER_ID}" - <<UserSpecificDocker
FROM hadoop-build-XX
RUN rm -f /var/log/faillog /var/log/lastlog
RUN groupadd --non-unique -g ${GROUP_ID} ${USER_NAME}
RUN useradd -g ${GROUP_ID} -u ${USER_ID} -k /root -m ${USER_NAME} -d "${DOCKER_HOME_DIR}"
RUN echo "${USER_NAME} ALL=NOPASSWD: ALL" > "/etc/sudoers.d/hadoop-build-XX-${USER_ID}"
